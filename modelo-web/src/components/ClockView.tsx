...
export const ClockView = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-[350px] text-center gap-6 p-6">
      ...
      <div className="text-4xl font-mono text-orange-700 mb-4">{formatTimer(timer)}</div>
      <div className="flex gap-4">
        <button
          onClick={() => setIsRunning(true)}
          disabled={isRunning}
          className={`px-4 py-2 rounded bg-orange-500 text-white font-semibold disabled:bg-orange-300`}
        >
          Iniciar
        </button>
        <button
